...
import com.yuand.smarteye.stock.dao.WareLocationDao;
...
public class WareLocationServiceImpl extends ServiceImpl<WareLocationDao, WareLocationEntity> implements WareLocationService {
    ...
    @Override
    public List<WareStockRespTo> transform(List<WareStockRespTo> tos) {
        for (WareStockRespTo to : tos) {
    ...
    //根据父id查询，其所有子区
    @Override
    public List<SonWareListVo> getSon(Long wlpId) {
        QueryWrapper<WareLocationEntity> wrapper = new QueryWrapper<WareLocationEntity>().eq("parent_wlid", wlpId);
        List<WareLocationEntity> wareLocationEntities = baseMapper.selectList(wrapper);
        List<SonWareListVo> res = new ArrayList<>();
        for (WareLocationEntity wareLocationEntity : wareLocationEntities) {
            SonWareListVo sonWareListVo = new SonWareListVo();
            BeanUtils.copyProperties(wareLocationEntity, sonWareListVo);
            res.add(sonWareListVo);
        }
        return res;
    }
}
